...
lib.EmailsInput = (function(utils) {
  ...
  function buildOptions(givenOptions) {
    ...
    options.placeholder = options.placeholder || 'add more phone ...'
  ...
  function addChip(refElement, email) {
    ...
    if (!isValidPhone(trimmedEmail))
      chip.classList.add('invalid')

    chip.innerHTML = '<span class="content p-0">'
          + trimmedEmail + '</span><a href="#" class="remove">×</a>'
    chip.innerHTML += '<input type="hidden" name="phonenumber[]" value="'+trimmedEmail+'">'
